...
> **版本兼容性说明**  
> 以下特殊主题功能需要设备端 HA 兼容性版本满足各自的最低要求。  
> 其中 `map/current/info` 与 `model/name` 需要版本号为 2 或以上，新版地图与路径能力需要版本号为 3 或以上。  
> 如果您的设备版本低于对应要求，这些功能将不可用。
...
| [地图与路径能力（HA v3+）](#地图与路径能力ha-v3) | `map/current/meta`<br>`path/current/meta`<br>`path/history/meta` | 机器人→HA（MQTT 元数据）<br>HA→机器人（HTTP 拉取正文） | 当前地图、当前路径、历史路径 | HA v3+ |
...
> **版本要求**：需要设备端HA兼容性版本号 ≥ 3

如果你接入的是当前正式的地图与路径能力，请优先参考 [地图与路径能力说明](./developers/map_path.md)。

该能力提供以下正式入口：

- 当前地图：`map/current/meta` + `/ha/map/current.json.gz`
- 当前路径：`path/current/meta` + `/ha/path/current.json.gz`
- 历史路径：`path/history/meta` + `/ha/path/history.json.gz`

旧的 `map/current/info` 说明继续保留，主要用于历史兼容；新的地图和路径能力已经拆成独立文档，开发者直接看新文档接入即可。
